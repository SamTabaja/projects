CREATE VIEW StudentsFollowing AS
	SELECT ssn AS student, student.program, branch.name AS branch
		FROM student
		LEFT JOIN branch ON branch.program = student.program;
		
CREATE VIEW FinishedCourses AS
	SELECT ssn AS student, course.code AS course, REPLACE(grade::text, '0', 'U') AS grade, credits
		FROM student
...
CREATE VIEW PathToGraduation AS
	...
	LEFT JOIN
		...
		WHERE classification = 'Mathmatics' GROUP BY student) AS MC
