window.onload = function() {
  ...
  getBookButton.onclick = () => {
    ...
    xhttp4.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (data.error) {
          errorMsg.innerHTML = "The book you entered does not exist";
          return;
        }
        if (title.value == "") {
          ...
          date.innerHTML = "Publish Data: " + data.publish_date[0];
          disc.innerHTML = "Description: " + data.description[0];
        }
      }
    };
  ...
  let appendData = (arr, data, parentDiv) => {
    ...
    for (let i = 0; i < arr; i++) {
      ...
      date.innerHTML += "Publish date: " + data[i].publish_date[0];
      disc.innerHTML += "Description: " + data[i].description[0];
      li.className += "mt-3";
